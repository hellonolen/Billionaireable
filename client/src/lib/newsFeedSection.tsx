...
import { DetailSquare, MiniStat } from "@/components/DashboardComponents";
...
export const NewsFeedDetail = () => {
  // Split news into groups of 2 for DetailSquares (3 columns total = 6 items)
  const newsGroups = [];
  for (let i = 0; i < FULL_NEWS_ALL.length; i += 2) {
    newsGroups.push(FULL_NEWS_ALL.slice(i, i + 2));
  }

  const titles = ["Top Stories", "Market Updates", "Additional News"];

  return (
    <>
      {newsGroups.map((group, groupIdx) => (
        <DetailSquare key={groupIdx} title={titles[groupIdx] || "More News"}>
            ...
            {group.map((item, idx) => {
              ...
              return (
                <div key={idx} className="border rounded-lg p-4" style={{ borderColor: COLORS.border }}>
                  ...
                  <p className="text-xs mb-2" style={{ color: COLORS.subt }}>
                    {item.summary}
                  </p>
